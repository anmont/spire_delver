...
void FSnapGridFlowEditor::FinalizeFlowGraphExecution(FFlowExecutionOutput& ResultNodeState, EFlowTaskExecutionResult& OutExecResult) {
	check(OutExecResult == EFlowTaskExecutionResult::Success);

	FString ErrorMessage;
	
	if (const USnapGridFlowConfig* SnapGridConfig = Cast<USnapGridFlowConfig>(GetDungeonConfig())) {
		if (const USnapGridFlowEditorSettings* SgfSettings = Cast<USnapGridFlowEditorSettings>(EditorSettings)) {
			if (SgfSettings->ModuleDatabase) {
				if (const USnapGridFlowModuleBoundsAsset* BoundsAsset = SgfSettings->ModuleDatabase->ModuleBoundsAsset.LoadSynchronous()) {
					// Resolve the graph and check if we it works
					...
					ResolveSettings.BaseTransform = FTransform::Identity;
					ResolveSettings.MaxResolveFrames = SnapGridConfig->MaxResolveFrames;
					ResolveSettings.MaxItemFitnessFrames = SnapGridConfig->MaxItemFitnessFrames;
					...
					USnapGridFlowModuleDatabase* ModuleDatabaseAsset = SgfSettings->ModuleDatabase.LoadSynchronous();
					const FSnapGridFlowModuleDatabaseImplPtr ModDB = MakeShareable(new FSnapGridFlowModuleDatabaseImpl(ModuleDatabaseAsset));
					const FSnapGridFlowModuleResolver ModuleResolver(ModDB, ResolveSettings);

					TArray<SnapLib::FModuleNodePtr> ModuleNodes;
					USnapGridFlowAbstractGraph* ResultAbstractGraph = ResultNodeState.State->GetState<USnapGridFlowAbstractGraph>(UFlowAbstractGraphBase::StateTypeID);
					if (!ModuleResolver.Resolve(ResultAbstractGraph, ModuleNodes)) {
						OutExecResult = EFlowTaskExecutionResult::FailHalt;
						ErrorMessage = "Failed to resolve modules";
					}
				}
				else {
					OutExecResult = EFlowTaskExecutionResult::FailHalt;
					ErrorMessage = "Bounds not assigned to ModuleDB";
				}
			}
		}
	}
	
	if (OutExecResult != EFlowTaskExecutionResult::Success) {
		ResultNodeState.ExecutionResult = OutExecResult;
		ResultNodeState.ErrorMessage = ErrorMessage;
	}
}
