...
void UEdGraph_DungeonProp::RebuildGraph(UObject* Owner, TArray<FPropTypeData>& OutProps, TArray<FDungeonGraphBuildError>& OutErrors) const {
    ...
    for (UEdGraphNode_DungeonMarker* MarkerNode : MarkerNodes) {
        ...
        DALib::GetChildNodes<UEdGraphNode_DungeonActorBase>(MarkerNode, ActorNodes);
        ActorNodes.Sort(DALib::ExecutionSortComparer());
        for (UEdGraphNode_DungeonActorBase* ActorNode : ActorNodes) {
            ...
            DALib::CloneUObjectArray(Owner, ActorNode->SelectionLogics, Prop.SelectionLogics);
            ...
            DALib::CloneUObjectArray(Owner, ActorNode->TransformLogics, Prop.TransformLogics);
            Prop.bUseTransformLogic = ActorNode->bUseTransformLogic;

            DALib::CloneUObjectArray(Owner, ActorNode->ProceduralTransformLogics, Prop.ProceduralTransformLogics);
            Prop.bUseProceduralTransformLogic = ActorNode->bUseProceduralTransformLogic;

            // Clone the spawn logic instance
            DALib::CloneUObjectArray(Owner, ActorNode->SpawnLogics, Prop.SpawnLogics);
            ...
            DALib::GetChildNodes<UEdGraphNode_DungeonMarkerEmitter>(ActorNode, EmitterNodes);
