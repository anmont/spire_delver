...
public:
    ...
    static void GetCellConnectedRooms(const FGridDungeonCell& Cell, TArray<int32>& ConnectedRooms);

    UFUNCTION(BlueprintCallable, Category = Dungeon)
    static void GetCellMSTRooms(const FGridDungeonCell& Cell, TArray<int32>& MSTRooms);

    UFUNCTION(BlueprintCallable, Category = Dungeon)
    static void GetCellCenter(const FGridDungeonCell& Cell, FVector& Center);
    ...
    UFUNCTION(BlueprintCallable, BlueprintPure, Category = Dungeon)
    static void GetBoundingBox(const TArray<FGridDungeonCell>& Cells, FRectangle& Bounds);

    /** Tries to find a free edge in the room bounds that does not contain a door */
    UFUNCTION(BlueprintCallable, BlueprintPure, Category = Dungeon)
    static void GetRoomFreeEdge(UGridDungeonModel* Model, const FGridDungeonCell& RoomCell, TArray<FVector>& FreeEdgeCenters,
                                TArray<float>& FreeEdgeAngles);

    UFUNCTION(BlueprintCallable, Category = Dungeon)
    static void GetDoorExits(const FGridDungeonCellDoor& Door, FVector& ExitA, FVector& ExitB);
