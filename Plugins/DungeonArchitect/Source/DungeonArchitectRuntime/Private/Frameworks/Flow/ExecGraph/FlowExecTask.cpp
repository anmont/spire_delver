...
void UFlowExecTask::Execute(const FFlowExecutionInput& Input, const FFlowTaskExecutionSettings& InExecSettings, FFlowExecutionOutput& Output) const {
...
TObjectPtr<UObject> FFlowExecNodeState::GetStateObject(const FName& InObjectID) const {
    const TObjectPtr<UObject>* SearchResult = StateObjects.Find(InObjectID);
    return SearchResult ? *SearchResult : nullptr; 
}

void FFlowExecNodeState::SetStateObject(const FName& InObjectID, const TObjectPtr<UObject>& InObject) {
    TObjectPtr<UObject>& StateRef = StateObjects.FindOrAdd(InObjectID);
    StateRef = InObject;
}
