...
void UFlowTilemapTaskOptimize::Execute(const FFlowExecutionInput& Input, const FFlowTaskExecutionSettings& InExecSettings, FFlowExecutionOutput& Output) const {
...
void UFlowTilemapTaskOptimize::DiscardDistantTiles(UFlowTilemap* Tilemap) const {
    ...
    {
        float DiscardDistance = FMath::Max(0, DiscardDistanceFromLayout);
        for (FFlowTilemapCell& Cell : Tilemap->GetCells()) {
            if (!DistanceFromLayout.Contains(Cell.TileCoord)) continue;
            if (Cell.bLayoutCell) continue;
            if (DistanceFromLayout[Cell.TileCoord] > DiscardDistance) {
