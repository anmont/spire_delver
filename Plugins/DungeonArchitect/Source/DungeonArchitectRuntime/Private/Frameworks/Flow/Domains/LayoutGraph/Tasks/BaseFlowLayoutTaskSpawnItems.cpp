...
void UBaseFlowLayoutTaskSpawnItems::Execute(const FFlowExecutionInput& Input, const FFlowTaskExecutionSettings& InExecSettings, FFlowExecutionOutput& Output) const {
...
int32 UBaseFlowLayoutTaskSpawnItems::GetSpawnCount(float Weight, const FRandomStream& Random) const {
    ...
    if (SpawnMethod == EridFlowTask_SpawnItemsMethod::RandomRange) {
        ...
        const float SpawnCountF = MinCount + (MaxCount - MinCount) * AdjustedWeight;
        SpawnCount = FMath::RoundToInt(SpawnCountF);
        SpawnCount = FMath::Clamp(SpawnCount, MinCount, MaxCount);
    }
