...
int32 FFlowAbstractGraphUtils::CreateUidRandomSeed(const FRandomStream* Random, TWeakObjectPtr<ADungeon> InDungeon) {
    int32 NamingSchemeSeed = Random ? Random->GetCurrentSeed() : 0;
    if (InDungeon.IsValid()) {
        NamingSchemeSeed = static_cast<int32>(HashCombine(static_cast<uint32>(NamingSchemeSeed), GetTypeHash(InDungeon->Uid)));
    }
    return NamingSchemeSeed;
}
