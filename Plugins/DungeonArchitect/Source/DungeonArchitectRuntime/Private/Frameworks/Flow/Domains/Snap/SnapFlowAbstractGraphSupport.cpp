...
bool FSnapGridFlowAbstractGraphConstraints::IsValid(const FFlowAbstractGraphQuery& InGraphQuery, const UFlowAbstractNode* Node,
                                                    const TArray<const UFlowAbstractNode*>& IncomingNodes, const TArray<const UFlowAbstractNode*>& OutgoingNodes) {
	...
	TSet<const UFlowAbstractNode*> AllConnectedNodes;
	AllConnectedNodes.Append(IncomingNodes);
	AllConnectedNodes.Append(OutgoingNodes);
	{
		...
		*/
		...
		// TODO: Pass in the sub node to the AllConnectedNodes list, rather than the composite node
		...
		for (const UFlowAbstractLink* ConnectedLink : ConnectedLinks) {
			...
			if (ConnectedLink->Source == Node->NodeId || ConnectedLink->SourceSubNode == Node->NodeId) {
				if (const UFlowAbstractNode* ConnectedNode = FFlowAbstractGraphPathUtils::TryGetSubNodeFromLink(InGraphQuery, ConnectedLink->Destination, ConnectedLink)) {
				...
				if (const UFlowAbstractNode* ConnectedNode = FFlowAbstractGraphPathUtils::TryGetSubNodeFromLink(InGraphQuery, ConnectedLink->Source, ConnectedLink)) {
					AllConnectedNodes.Add(ConnectedNode);
				}
			}
		} 
	...
	for (const UFlowAbstractNode* ConnectedNode : AllConnectedNodes) {
		check(ConnectedNode->MergedCompositeNodes.Num() == 0);
	}
...
void FSGFNodeGroupGenLib::BuildNodeGroup(const FFlowAbstractGraphQuery& InGraphQuery,
		...
		if (bHostsSource) {
			if (OutGroup.GroupEdgeNodes.Contains(Source)) {
				...
				if (SourceNode && DestinationNode) {
					OutConstraintLinks.Add(FFAGConstraintsLink(SourceNode, DestinationNode, EFAGConstraintsLinkDirection::Incoming));
				}
			}
		}
		else if (bHostsDest) {
			if (OutGroup.GroupEdgeNodes.Contains(Destination)) {
				...
				if (!DestinationNode) DestinationNode = InGraphQuery.GetSubNode(Destination);
				if (SourceNode && DestinationNode) {
					OutConstraintLinks.Add(FFAGConstraintsLink(DestinationNode, SourceNode, EFAGConstraintsLinkDirection::Outgoing));
				}
			}
		}
	}
	...
	for (const UFlowAbstractNode* const IncomingNode : InIncomingNodes) {
		...
		if (InnerNodePtr) {
				...
				OutConstraintLinks.Add(FFAGConstraintsLink(InnerNode, IncomingNode, EFAGConstraintsLinkDirection::Incoming));	
		}
	}
}
