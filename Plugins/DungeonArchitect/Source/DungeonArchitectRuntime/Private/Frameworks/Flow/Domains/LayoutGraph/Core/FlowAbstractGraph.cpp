...
UFlowAbstractNode* UFlowAbstractGraphBase::CreateNode(const FRandomStream& InRandom) {
    UFlowAbstractNode* Node = NewObject<UFlowAbstractNode>(this, GetNodeClass());
    Node->NodeId = FRandomUtils::CreateGuid(InRandom);
    GraphNodes.Add(Node);
    return Node;
}
...
UFlowAbstractLink* UFlowAbstractGraphBase::GetLink(const FGuid& SourceNodeId, const FGuid& DestNodeId,
    ...
    for (UFlowAbstractLink* Link : GraphLinks) {
        check(Link);
        if (Link && Link->Source == SourceNodeId && Link->Destination == DestNodeId) {
            return Link;
        }
        if (bIgnoreDirection) {
            if (Link && Link->Source == DestNodeId && Link->Destination == SourceNodeId) {
                return Link;
            }
        }
    }
...
UFlowAbstractLink* UFlowAbstractGraphBase::CreateLink(const FGuid& SourceNodeId, const FGuid& DestNodeId, const FRandomStream& InRandom) {
    ...
    Link->LinkId = FRandomUtils::CreateGuid(InRandom);
