...
namespace {
    ...
    public:
        ...
        int32 Calculate(const TArray<FString>& InMarkerNames, int32 MaxProcessingSteps) const {
            TFunction<bool(TMap<FString, int32>& MarkerFreq, TArray<TPair<UPlaceableMarkerAsset const*, int32>>& Assets)> SolveBinPacking;

            int32 NumFramesProcessed{};
            
            SolveBinPacking = [&](TMap<FString, int32>& MarkerFreq, TArray<TPair<UPlaceableMarkerAsset const*, int32>>& Assets) -> bool {
                if (MarkerFreq.Num() == 0) return true; // All markers are allocated
                if (NumFramesProcessed >= MaxProcessingSteps) {
                    return false;
                }
                NumFramesProcessed++;
                ...
                for (auto& Asset : Assets) {
                    if (Asset.Value > 0) {
                        for (const auto& MarkerType : Asset.Key->MarkerNames) {
                            if (MarkerFreq.Contains(MarkerType) && MarkerFreq[MarkerType] > 0) {
                                // Allocate marker to asset
                                MarkerFreq[MarkerType]--;
                                if (MarkerFreq[MarkerType] == 0) {
                                    MarkerFreq.Remove(MarkerType);
                                }
                                Asset.Value--;

                                // Recursively allocate remaining markers
                                if (SolveBinPacking(MarkerFreq, Assets)) {
                                    return true;
                                }
                                ...
                                // Backtrack if allocation fails
                                Asset.Value++;
                                if (MarkerFreq.Contains(MarkerType)) {
                                    MarkerFreq[MarkerType]++;
                                } else {
                                    MarkerFreq.Add(MarkerType, 1);
                                }
                            }
                        }
                    }
            ...
            TMap<FString, int32> MarkerFrequency;
            for (const auto& Marker : InMarkerNames) {
                if (MarkerFrequency.Contains(Marker)) {
                    MarkerFrequency[Marker]++;
                } else {
                    MarkerFrequency.Add(Marker, 1);
                }
            }
            // Sort assets based on the number of marker types they hold (ascending order)
            TArray<TPair<UPlaceableMarkerAsset const*, int32>> SortedAssets;
            for (const auto& Elem : ModuleMarkers) {
                SortedAssets.Add(Elem);
            }
            SortedAssets.Sort([](const TPair<UPlaceableMarkerAsset const*, int32>& A, const TPair<UPlaceableMarkerAsset const*, int32>& B) {
                return A.Key->MarkerNames.Num() < B.Key->MarkerNames.Num();
            });
            
            const int32 FitnessCost = SolveBinPacking(MarkerFrequency, SortedAssets) ? 0 : 100000;
            return FitnessCost;
        }
...
void FSnapGridFlowModuleResolver::GetCandidates(FSGFModuleResolveState& InResolveState, UFlowAbstractNode* InNode, int32 InDepth, const FSGFModuleAssembly& InAssembly, TArray<FModuleFitCandidate>& OutCandidates) const {
    ...
    for (int32 ModuleIdx : ModuleIndices) {
        ...
        int32 ItemFitness = ItemFitnessCalculator.Calculate(DesiredNodeMarkers, Settings.MaxItemFitnessFrames);
