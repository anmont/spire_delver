...
#if WITH_EDITOR
    // Move the spawned actors to the correct subfolder
    if (InDungeon) {
        const FString ConnectionsFolderPath = InDungeon->ItemFolderPath.ToString() + FDungeonArchitectConstants::DungeonFolderPathSuffix_SnapConnections;
        for (TWeakObjectPtr<AActor> SpawnedInstance : SpawnedInstances) {
            if (SpawnedInstance.IsValid()) {
                SpawnedInstance->SetFolderPath(FName(ConnectionsFolderPath));
            }
        }
    }
#endif // WITH_EDITOR
...
void ASnapConnectionActor::DestroyConnectionInstance() {
    for (const TWeakObjectPtr<AActor> SpawnedInstance : SpawnedInstances) {
        if (AActor* InstanceActor = SpawnedInstance.Get()) {
            InstanceActor->Destroy();
        }
    }
