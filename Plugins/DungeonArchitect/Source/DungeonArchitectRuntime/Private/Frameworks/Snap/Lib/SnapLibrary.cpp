...
TSoftObjectPtr<UWorld> SnapLib::IModuleDatabaseItem::GetThemedLevel(const FName& InThemeName) const {
...
bool SnapLib::FSnapGraphGenerator::ModuleOccludes(const FModuleNodePtr& ModuleNode, const SnapLib::ISnapGraphNodePtr& MissionNode, const TArray<FOcclusionEntry>& OcclusionList) {
    ...
    for (const FDungeonNegationVolumeState& NegationVolume : StaticState.NegationVolumes) {
