...
void UGridFlowTilemapTaskInitialize::Execute(const FFlowExecutionInput& Input, const FFlowTaskExecutionSettings& InExecSettings, FFlowExecutionOutput& Output) const {
...
void UGridFlowTilemapTaskInitialize::LookAhead(const FFlowExecutionInput& Input) const {
    Super::LookAhead(Input);

    const TSharedPtr<FGridFlowTilemapDomain> TilemapDomain = StaticCastSharedPtr<FGridFlowTilemapDomain>(Input.Domain.Pin());
    if (TilemapDomain.IsValid()) {
        TilemapDomain->CachedChunkSize = TilemapSizePerNode;
        TilemapDomain->CachedLayoutPadding = LayoutPadding;
    }
}

UGridFlowTilemap* UGridFlowTilemapTaskInitialize::BuildTilemap(UGridFlowAbstractGraph* Graph, const FRandomStream& Random) const {
    const FFlowAbstractGraphQuery GraphQuery(Graph);
    FIntPoint LayoutGridSize = Graph->GridSize;
...
void UGridFlowTilemapTaskInitialize::PerturbRoomSizes(FGridFlowTilemapNodes& TileNodes, UGridFlowTilemap* Tilemap,
    ...
    int32 ClampedCorridorLaneWidth = FMath::Max(1, CorridorLaneWidth);
    FIntPoint LayoutGridSize = Graph->GridSize;
    // Perturb horizontally
    {
        int32 MaxPerturbAmount = FMath::FloorToInt((TilemapSizePerNode.X - 2 - ClampedCorridorLaneWidth) * 0.5f);
    ...
    // Perturb vertically
    {
        int32 MaxPerturbAmount = FMath::FloorToInt((TilemapSizePerNode.Y - 2 - ClampedCorridorLaneWidth) * 0.5f);
        MaxPerturbAmount = FMath::Max(0, MaxPerturbAmount);
...
void UGridFlowTilemapTaskInitialize::FixCorridorSizes(FGridFlowTilemapNodes& TileNodes, const FFlowAbstractGraphQuery& GraphQuery) const {
    int32 ClampedCorridorLaneWidth = FMath::Max(1, CorridorLaneWidth);
    for (FGridFlowTilemapNodeInfo& TileNode : TileNodes.GetCells()) {
        ...
        if (Node && Node->FindOrAddDomainData<UFANodeTilemapDomainData>()->RoomType == EGridFlowAbstractNodeRoomType::Corridor) {
            ...
            if (bVertical) {
                TileNode.x0 = FMath::Max(TileNode.x0, TileNode.midX - ClampedCorridorLaneWidth);
                TileNode.x1 = FMath::Min(TileNode.x1, TileNode.midX + ClampedCorridorLaneWidth);
            }
            else {
                TileNode.y0 = FMath::Max(TileNode.y0, TileNode.midY - ClampedCorridorLaneWidth);
                TileNode.y1 = FMath::Min(TileNode.y1, TileNode.midY + ClampedCorridorLaneWidth);
            }
        }
    }
}
...
void UGridFlowTilemapTaskInitialize::BuildCaves(FGridFlowTilemapNodes& TileNodes, UGridFlowTilemap* Tilemap,
                                                 const FFlowAbstractGraphQuery& GraphQuery, const FRandomStream& Random) const
{
    CalculateDistanceFromMainPath(TileNodes, Tilemap, GraphQuery, {EGridFlowAbstractNodeRoomType::Cave});
