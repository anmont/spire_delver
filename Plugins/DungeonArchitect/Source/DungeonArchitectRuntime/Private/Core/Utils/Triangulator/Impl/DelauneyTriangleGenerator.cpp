...
const float DA_DEL_EPSILON = 1e-6f;
...
int CircumCircle(double xp, double yp,
    ...
    if (fabsy1y2 < DA_DEL_EPSILON && fabsy2y3 < DA_DEL_EPSILON)
        return false;

    if (fabsy1y2 < DA_DEL_EPSILON) {
        m2 = -(x3 - x2) / (y3 - y2);
        mx2 = (x2 + x3) / 2.0;
        my2 = (y2 + y3) / 2.0;
        *xc = (x2 + x1) / 2.0;
    ...
    else if (fabsy2y3 < DA_DEL_EPSILON) {
        m1 = -(x2 - x1) / (y2 - y1);
        mx1 = (x1 + x2) / 2.0;
        my1 = (y1 + y2) / 2.0;
        *xc = (x3 + x2) / 2.0;
    ...
    *rsqr = dx * dx + dy * dy;
    ...
    return ((drsqr - *rsqr) <= DA_DEL_EPSILON ? true : false);
}
