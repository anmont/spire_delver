...
TArray<int32> UGridDungeonQuery::GetCells() {
    TArray<int32> Result;
    if (Model) {
        for (const FGridDungeonCell& Cell : Model->Cells) {
...
TArray<int32> UGridDungeonQuery::GetCellsOfType(EGridDungeonCellType CellType) {
    TArray<int32> Result;
    if (Model) {
        for (const FGridDungeonCell& Cell : Model->Cells) {
...
void UGridDungeonQuery::GetCellDimension(int32 CellId, FVector& OutCenter, FVector& OutSize) {
    if (Model && Config) {
        ...
        FGridDungeonCell Cell = GetCell(CellId);
        if (Cell.CellType == EGridDungeonCellType::Unknown) {
...
FVector GetCellCenter(const FGridDungeonCell& Cell, const FVector& GridSize) {
...
void UGridDungeonQuery::GetPathBetweenCells(int32 CellA, int32 CellB, TArray<int32>& OutResult, bool& bOutSuccess) {
    ...
    while (!Queue.IsEmpty()) {
        ...
        const FGridDungeonCell& Cell = CellLookup[TopCellId];
        FVector GridSize = Config->GridCellSize;
        ...
        AdjacentCells.Sort([&](int32 CellIdA, int32 CellIdB) {
            const FGridDungeonCell& AdjCellA = CellLookup[CellIdA];
            const FGridDungeonCell& AdjCellB = CellLookup[CellIdB];

            const float DistASq = (CellCenter - GetCellCenter(AdjCellA, GridSize)).SizeSquared();
            const float DistBSq = (CellCenter - GetCellCenter(AdjCellB, GridSize)).SizeSquared();
        ...
        for (const int32 AdjacentCellId : AdjacentCells) {
            ...
            const FGridDungeonCell& AdjacentCell = CellLookup[AdjacentCellId];
            ...
            if (Cell.CellType == EGridDungeonCellType::Room || AdjacentCell.CellType == EGridDungeonCellType::Room) {
...
void UGridDungeonQuery::GetFurthestRoomsRecursive(const LongestPathSearchNode& Top, int32 StartRoomId,
    ...
    FGridDungeonCell& TopCell = CellLookup[Top.CellId];
    ...
    for (int32 ChildId : TopCell.FixedRoomConnections) {
        ...
        FGridDungeonCell& ChildCell = CellLookup[ChildId];
...
void UGridDungeonQuery::GetFurthestRooms(int32& OutRoomA, int32& OutRoomB) {
    ...
    for (const FGridDungeonCell& StartCell : Model->Cells) {
        if (StartCell.CellType != EGridDungeonCellType::Room) {
            continue;
        }
        ...
        while (!Queue.IsEmpty()) {
            ...
            FGridDungeonCell& FrontCell = CellLookup[Front.CellId];
...
EGridDungeonCellType UGridDungeonQuery::GetCellType(int32 CellId) {
...
int32 UGridDungeonQuery::GetRandomCellOfType(EGridDungeonCellType CellType) {
...
int32 UGridDungeonQuery::GetRandomCellOfTypeFromStream(EGridDungeonCellType CellType, FRandomStream& RandomStream) {
    auto Cells = GetCellsOfType(CellType);
    if (Cells.Num() == 0) {
        UE_LOG(LogGridDungeonQuery, Error,
               TEXT("Cannot fetch a random cell of specified type as no cells exist in the model"));
...
bool UGridDungeonQuery::ContainsStairBetween(int32 CellA, int32 CellB) {
    ...
    for (const FGridDungeonStairInfo& Stair : Model->Stairs) {
...
bool UGridDungeonQuery::ContainsDoorBetween(int32 CellA, int32 CellB) {
    ...
    for (const FGridDungeonCellDoor& Door : Model->Doors) {
...
FGridDungeonStairInfo UGridDungeonQuery::GetStairBetween(int32 CellA, int32 CellB) {
    if (Model) {
        for (const FGridDungeonStairInfo& Stair : Model->Stairs) {
            if ((Stair.OwnerCell == CellA && Stair.ConnectedToCell == CellB) ||
                (Stair.OwnerCell == CellB && Stair.ConnectedToCell == CellA)) {
    ...
    return FGridDungeonStairInfo();
}

FGridDungeonCellDoor UGridDungeonQuery::GetDoorBetween(int32 CellA, int32 CellB) {
    if (Model) {
        for (const FGridDungeonCellDoor& Door : Model->Doors) {
            if ((Door.AdjacentCells[0] == CellA && Door.AdjacentCells[1] == CellB) ||
                (Door.AdjacentCells[0] == CellB && Door.AdjacentCells[1] == CellA)) {
    ...
    return FGridDungeonCellDoor();
}

FVector UGridDungeonQuery::GetOpeningPointBetweenAdjacentCells(int32 CellAId, int32 CellBId) {
    FGridDungeonCell CellA = GetCell(CellAId);
    FGridDungeonCell CellB = GetCell(CellBId);
    if (!Model || !Config) {
        UE_LOG(LogGridDungeonQuery, Error, TEXT("Invalid model state"));
    ...
    if (CellA.CellType == EGridDungeonCellType::Room || CellB.CellType == EGridDungeonCellType::Room) {
        if (ContainsDoorBetween(CellAId, CellBId)) {
            FGridDungeonCellDoor Door = GetDoorBetween(CellAId, CellBId);
        ...
        if (ContainsStairBetween(CellAId, CellBId)) {
            FGridDungeonStairInfo Stair = GetStairBetween(CellAId, CellBId);
...
FGridDungeonCell UGridDungeonQuery::GetCell(int32 CellId) {
    if (Model) {
        for (const FGridDungeonCell& Cell : Model->Cells) {
            if (Cell.Id == CellId) {
                return Cell;
    ...
    return FGridDungeonCell();
}

void UGridDungeonQuery::GenerateCellLookup(TMap<int32, FGridDungeonCell>& OutLookup) {
    OutLookup.Reset();
    if (!Model) {
