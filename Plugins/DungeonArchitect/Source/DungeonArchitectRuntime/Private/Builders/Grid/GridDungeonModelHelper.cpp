...
void UGridDungeonModelHelper::GetCellConnectedRooms(const FGridDungeonCell& Cell, TArray<int32>& ConnectedRooms) {
    ConnectedRooms = Cell.ConnectedRooms;
}

void UGridDungeonModelHelper::GetCellMSTRooms(const FGridDungeonCell& Cell, TArray<int32>& MSTRooms) {
    MSTRooms = Cell.FixedRoomConnections;
}

void UGridDungeonModelHelper::GetCellCenter(const FGridDungeonCell& Cell, FVector& Center) {
...
void UGridDungeonModelHelper::GetBoundingBox(const TArray<FGridDungeonCell>& Cells, FRectangle& Bounds) {
    int32 minX = INT_MAX;
    int32 maxX = -INT_MAX;
    int32 minY = INT_MAX;
    int32 maxY = -INT_MAX;
...
void UGridDungeonModelHelper::GetRoomFreeEdge(UGridDungeonModel* Model, const FGridDungeonCell& RoomCell,
                                              TArray<FVector>& FreeEdgeCenters, TArray<float>& FreeEdgeAngles) {
    const TArray<FGridDungeonCellDoor>& Doors = Model->DoorManager.GetDoors();
...
void UGridDungeonModelHelper::GetDoorExits(const FGridDungeonCellDoor& Door, FVector& ExitA, FVector& ExitB) {
...
FVector UGridDungeonModelHelper::GetRandomCellLocation(UGridDungeonModel* Model, UGridDungeonConfig* Config) {
    ...
    FGridDungeonCell Cell = Model->Cells[FMath::Rand() % CellCount];
    FVector Center;
